...
import { getCatetory, getArticle, addCatetory, deleteCatetory, deleteArticle } from './utils/utils'
import {
    NavLink,
    withRouter
    ...
    return (
        <>
            <div className="edit">
                ...
                <Button className="edit_btn" onClick={()=>{toAdd("","")}} >
                    新增
                </Button>
                <div className="edit_catetory">
                    {
                        catetory.map(item => {
                            return (
                                <div className={`catetory_item ${catetoryData.id === item.id ? "selected" : ""}`} onClick={() => queryArticle(item)}>{item.name}
                            ...
                            return (
                                <div className="catetory_item">
                                    ...
                                    <div className="item_set">
                                        <div className="set_edit" onClick={()=>toAdd(item,1)}>编辑</div>
                                        <div className="set_delete" style={{ marginLeft: "10px" }} onClick={()=>{setArticleData(item);setVisible3(true)}}>删除</div>
                                    </div>
                                </div>
                            )
                        })
                ...
                <Modal style={{ top: 300 }} title="提示" visible={visible3} onOk={()=>{_deleteArticle(articleData)}} onCancel={() => { setVisible3(false) }}>
                    {`确定删除 ${articleData.title} 文章吗？`}
                </Modal>
                <Modal style={{ top: 300 }} title="提示" visible={visible2} onOk={_deleteCatetory} onCancel={() => { setVisible2(false) }}>
                    {`确定删除 ${catetoryData.name} 分类吗？`}
                </Modal>
                <Modal style={{ top: 300 }} title="新增" visible={visible} onOk={handleOk} onCancel={() => { setVisible(false) }}>
